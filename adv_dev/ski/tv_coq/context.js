...
{
	myContext.mode = 'tv_etor';
	myContext.option_tv = true;
	
	myContext.nb_ranking = 10;
	myContext.nb_oncourse = 6;
	myContext.delay_ranking = 12000;

	// Neutralisation ...
	myContext.OnOption = function() {};
	myContext.OnEpreuve = function() {};
	myContext.OnFooter = function() {};
	
	myContext.htmlClassGroup2 = function(class_group)
	{
	...
	{ 
		...
		if (container != null && typeof container === 'object')
		{
			...
			html += '<h1>Coq d\'Or 2025</h1>';
			html += '<h2>'+tEvenement.GetCell('Commentaire_live', 0)+'</h2>';
			html += '<h3>Manche '+ski.GetCodeManche()+' - Résultats Officieux</h3>';
		 
			html += '<div id="container_marquee">';
			html += '<div id="marquee"><span id="msg">&nbsp;</span></div>';
			html += '</div>';
			html += '<div id="label_marquee">&nbsp;</div>';
			html += '</div>';
		
			container.innerHTML = html;
			myContext.container_msg = document.getElementById('msg');
		
			container.addEventListener('click', function() { adv.ToggleFullscreen(); return false });
		}
	}
	...
	myContext.getColumnsRanking = function()
	{
		...
		ranking.push({ name: "Identite", label: 'Identité', "class": 'text-start major', lg: 32});
		ranking.push({ name: "Categ", label: 'Cat.', "class": 'text-center minor', lg: 3});
		ranking.push({ name: "Sexe", label: 'S.', "class": 'text-center minor', lg: 1 });
		ranking.push({ name: "Equipe", label: 'E.S.F.', "class": 'text-start minor', lg: 18 });
