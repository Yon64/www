...
function OnCommandCompetitionLoad(objJSON) 
{
	...
	for (let r = 0; r < tCompetitions.GetNbRows(); r++)
	{
		...
		const label = tCompetitions.GetCell('Nom', r);
		...
		html += '<td>'+tCompetitions.GetCell('key',r)+'</td>';
		html += '<td>'+activite+'</td>';
	...
	document.getElementById("container_ws_live").innerHTML = html;

	[].forEach.call(document.querySelectorAll('#container_ws_live tr td a'), function(el) {
