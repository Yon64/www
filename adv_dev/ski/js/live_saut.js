...
{
	if (typeof wsMain.notify_ranking === 'object')
	{
...
//	alert("OnBroadcastBibNext :"+JSON.stringify(objJSON));
	...
	{
		wsMain.bib_current = parseInt(objJSON.bib);
...
function OnBroadcastBibSave(objJSON) 
{
	if (ski.GetCodeActivite() == 'SAUT' && objJSON.bib !== null)
	{
		wsMain.bib_save = parseInt(objJSON.bib);
	}
}
...
function OnCommandRaceLoad(objJSON) 
{
	...
	const tEpreuve_Nordique = ski.GetTable('Epreuve_Nordique');
	if (typeof tEpreuve_Nordique == 'object')
	{
		if (tEpreuve_Nordique.GetCell('Type_equipe', 0) == 'V' )
			ski.vent = true;
	}
...
//	document.getElementById("title").innerHTML = tEvenement.GetCell('Nom', 0);
	document.getElementById("sub_title").innerHTML = 'Manche '+ski.GetCodeManche();
}
...
function SetArrayColumns() 
{
	...
	wsMain.array_columns.push({ "name" : "Clt", "label" : GetTraduction('rk'), "style" : "rank", "context" : "manche" });
	wsMain.array_columns.push({ "name" : "Dossard", "label" : GetTraduction('bib'), "style" : "bib" });
	wsMain.array_columns.push({ "name" : "Identite", "label" : GetTraduction('name'), "style" : "name" });
	wsMain.array_columns.push({ "name" : "Sexe", "label" : GetTraduction('sex'), "style" : "name" });
	wsMain.array_columns.push({ "name" : "Categ", "label" : GetTraduction('categ'), "style" : "name" });
...
{
	if (typeof wsMain.notify_epreuve === 'object')
		return parseInt(wsMain.notify_epreuve.epreuve);
	else
		return 1;
}
...
function SetBodyEpreuveSAUT()
{
	...
	for (var i=0;i<wsMain.array_columns.length;i++)
	{
		if (wsMain.array_columns[i].hasOwnProperty('context'))
		{
			...
			{
				var columnName = wsMain.array_columns[i].name
				if (manche == 1)
					columnName += "1";
			...
			else if (columnContext == 'finish_start')
			{
				...
				{				
					...
					html += '<th class="text_end" data-sort-header="Vent1">V1</th>';
					html += '<th class="text_end" data-sort-header="NoteVen1t">P.V1</th>';
	...
	for (let r = rMin; r<=rMax; r++)
	{
		...
		html += '<tr ';
		if (parseInt(bib) == wsMain.bib_save)
			html += ' class="bib_save" ';
		else if (parseInt(bib) == wsMain.bib_current)
		...
		for (var i=0;i<wsMain.array_columns.length;i++)
		{
			if (wsMain.array_columns[i].hasOwnProperty('context'))
			{
				...
				else if (columnContext == 'finish_start')
				{
					...
					var vent = tRanking.GetCellFormat('Vent'+manche, r);
					if (vent != '')
						vent = (parseFloat(lg)/10).toFixed(1);
					...
					if (manche == 1)
					{
						...
						html += '<th class="text_end">'+vent+'</th>';
						html += '<th class="text_end">'+tRanking.GetCellFormat('NoteVent1', r, '1')+'</th>';
...
function Init()
{
	...
	wsMain.title = 'Saut à Ski';
	if (urlParams.has('title'))
		wsMain.title = urlParams.get('title');
	document.getElementById("title").innerHTML = wsMain.title+'<br>&nbsp;';
...
//		alert('scrolling='+wsMain.scrolling);
	...
	wsMain.SetCommand('<ping>', OnCommandPing);
	wsMain.SetCommand('<pdf>', OnCommandPDF);
	
	wsMain.SetCommand('<*>', OnCommandDebug);
	wsMain.SetCommand('*', OnCommandDebug);
	
	// Broadcast Notification
	wsMain.SetCommand('<bib_time>', OnBroadcastBibTime);
	wsMain.SetCommand('<bib_time_lap>', OnBroadcastBibTimeLap);
	wsMain.SetCommand('<bib_next>', OnBroadcastBibNext);
	wsMain.SetCommand('<bib_save>', OnBroadcastBibSave);
	wsMain.SetCommand('<ranking_load>', OnBroadcastRankingLoad);
