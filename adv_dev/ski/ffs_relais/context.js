function InitContext(paramsJSON)
{
	...
	myContext.getColumnsRanking = function()
	{
		...
		ranking.push({ name: "Nom", label: 'Equipe', "class": 'text-start hot', lg: 30, fn_value : myContext.GetCellValue});
		ranking.push({ name: "Comite", label: 'CS', "class": 'text-center', lg: 3});
		ranking.push({ name: "Club", label: 'Club', "class": 'text-center', lg: 15 });
		...
		ranking.push({ name: "lap1", label: 'Lap1', "class" : 'text-end', lg: 7, fn_value : myContext.GetCellLapValue });
		ranking.push({ name: "lap2", label: 'Lap2', "class" : 'text-end', lg: 7, fn_value : myContext.GetCellLapValue });
		ranking.push({ name: "lap3", label: 'Lap3', "class" : 'text-end', lg: 7, fn_value : myContext.GetCellLapValue });
		ranking.push({ name: "lap4", label: 'Lap4', "class" : 'text-end', lg: 7, fn_value : myContext.GetCellLapValue });
	...
	myContext.GetCellLapValue = function(tRanking, column, i, rowType)
	{
		...
		if (parseInt(lap) <= parseInt(lapCurrent))
			return tRanking.GetCellChrono('Tps1_lap'+lap,i,'HHMMSSD')+' ('+tRanking.GetCellRank('Clt1_lap'+lap,i)+')';
		else
			return '';
	}
	...
	myContext.OnLive = function()
	{
		...
		const nbEquipier = tEpreuve.GetCell('Nb_participants', 0);
		
		for (let l=1;l<=nbEquipier;l++)
			VerifColumnsLap(tRanking, l);
		
		for (let row=0;row<tRanking.GetNbRows();row++)
		{
			for (let l=1;l<=nbEquipier;l++)
			{
				 SetTpsCumul(tRanking, row, l);
			}
		}
		
		for (let l=1;l<=nbEquipier;l++)
		{
			tRanking.SetRanking('Clt1_lap'+String(l), 'Tps1_lap'+String(l));
			tRanking.ComputeDiffTime('Tps1_lap'+String(l), 'Diff1_lap'+String(l));

			tRanking.SetRanking('Clt1_cumul'+String(l), 'Tps1_cumul'+String(l));
			tRanking.ComputeDiffTime('Tps1_cumul'+String(l), 'Diff1_cumul'+String(l));
		}
		...
		// Relais ...
		for (let equipier=1;equipier<=nbEquipier;equipier++)
		{
			html += htmlCode('<tr>', {class_name:'lap', data:{'id':'label_lap'+String(equipier)} });
			html += '\n\t';
			html += htmlCode('<th>', {class_name:'text-center', data:{colspan:'100%'} });
			html += '<img src="./img/16x16_chronometer.png"/>&nbsp;Relais '+String(equipier);
			html += htmlCode('</th>')
			html += '\n';
			html += htmlCode('</tr>');
			html += '\n';
			html += myContext.OnLiveLap(equipier);
		}
	...
	{
		const tRanking = GetTableContextRanking();
		tRanking.OrderBy('Tps1_cumul'+String(lap)+', Dossard');
		
		var html = '';
		for (let i=0;i<tRanking.GetNbRows();i++)
		{
			if (tRanking.GetCellInt('Tps1_cumul'+String(lap),i, adv.chrono.KO) < adv.chrono.OK)
				break;
