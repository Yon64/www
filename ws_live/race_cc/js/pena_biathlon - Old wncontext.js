...
{
	wsContext.pena_index = parseInt(indexPena);
	var ValuePenalite = 0;
	var html;
	
	html  = '<h1 class="text-center text-info bg-dark">Notation Tir n°'+wsContext.pena_index.toString()+'</h1>';
...
function onBibsaisiPena()
{
	...
	var identite = GetIdentite(bib);
	DoNotation(wsContext.pena_index, bib, identite);
}
...
function onSendPena(bibSelected)
{
	...
	const cmd = { key : '<bib_penalty>', bib : bibSelected, col : wsContext.pena_index, value : nbKo, signature : 'ws' };
	wsContext.wsCommand.send(JSON.stringify(cmd));
	document.getElementById("message").innerHTML = '&nbsp;&nbsp;&nbsp;&nbsp; Envoi de '+nbKo+' pénalités pour le dossard: '+bibSelected+' au Tir N°'+wsContext.pena_index+' Ok ...';
...
function onBibNext()
{
	...
	if (nb == 0 || nb == ski.GetSecteurNbPorte(wsContext.secteur))
		SetBibStep(1);
	else
		alert('Saisie non complète ! '+nb.toString()+' / '+ski.GetSecteurNbPorte(wsContext.secteur).toString());
}
