...
function OnFlowOnCourse(objJSON) 
{
	...
	if (objJSON.epreuve !== undefined && objJSON.epreuve != '')
	{
		if ((objJSON.epreuve != currentEpreuve) && (wsMain.all_categ != 1) && (currentEpreuve != '')) 
		{
	...
	// Rafraichissement si le nombre de personnes en course change
	if ( tOnCourse && (wsMain.scrolling > 0) && tOnCoursePrev && (tOnCourse.GetNbRows() != tOnCoursePrev.GetNbRows()) )
		SetBodyEpreuve(true);
}
...
// On compte le nombre de bateaux arrivés pour la gestion de l'affichage sans les bateaux avec horaire dep. et le nombre dans le classement voulu pour le podium
	...
	const tOnCourseRef = wsMain.notify_on_course;
	for (let r = 0; r<=tRanking.GetNbRows()-1; r++)
	{
		let hasRank = false;
		const bib = tRanking.GetCell('Dossard', r);
		let isOnCourse = (tOnCourseRef && typeof tOnCourseRef === 'object' && tOnCourseRef.GetIndexRow('bib', bib) >= 0);
		...
		if (isOnCourse && !hasRank) nb_oncourse++;
	}
...
// Selon les options on affiche tout avec horaires ou que les bateaux arrivés ou le podium
	...
	{
		nb_affiche=nb_finish+nb_oncourse;
	...
	if (wsMain.scrolling > 0)
	{
		...
		{
			const maxRows = wsMain.scrolling;
			let lastOnCourseIndex = -1;
			if (typeof tOnCourseRef === 'object' && tOnCourseRef && tOnCourseRef.GetNbRows() > 0) 
			{
				for (let i = 0; i < tOnCourseRef.GetNbRows(); i++) 
				{
					const bib = tOnCourseRef.GetCell('bib', i);
					const r = tRanking.GetIndexRow('Dossard', bib);
					if (r > lastOnCourseIndex) {
						lastOnCourseIndex = r;
					}
				}
			}

			if (lastOnCourseIndex >= 0) 
			{
				rMax = lastOnCourseIndex;
				rMin = Math.max(0, rMax - maxRows + 1);
				if (rMin === 0 && nb_affiche >= maxRows)
					rMax = maxRows - 1;
			} 
			else 
			{
...
let lg_tab=-1;
	for (let r = rMin; r<=rMax; r++)
	{
		...
		for (var i=0;i<wsMain.array_columns.length;i++)
		{
			if (wsMain.array_columns[i].hasOwnProperty('context'))
			{
				...
				else if (columnContext == 'finish_start')
				{
					...
					{
						const idxInOnCourse = tOnCourseRef.GetIndexRow('bib', bib);
						html_cour += '<td class="'+wsMain.array_columns[i].style+'" data-oncourse="1">'+tOnCourseRef.GetCellChrono('time', idxInOnCourse, wsMain.fmtChronoOnCourse)+'</td>';
					}
					else
		...
		html_cour += '</tr>';
		if (((!((wsMain.last_finish>0)&&(classement==0))  )  && (!((wsMain.horaire==0)&&(classement==0)&&(nb_finish>0)) )) || isOnCourse)
			html += html_cour;
		else
