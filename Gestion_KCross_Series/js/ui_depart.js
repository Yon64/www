...
_getMainActionButtons(heat){return`
        ...
        `;}
async initPenalty(heatId){try{const heatToStart=this.upcomingHeats.find(h=>h.id===parseInt(heatId,10));if(!heatToStart)return;const penaltyUpdatePromises=heatToStart.slots.filter(slot=>slot.participant&&slot.participant.id).map(slot=>{return this.persistenceManager.updatePenalite(heatId,slot.prioDepart,'0'.repeat(heatToStart.gateCount+2));});await Promise.all(penaltyUpdatePromises);}catch(error){notifyError(`${window.APP_TEXTS.depart.alertErrorStartingHeat} ${error.message}`);}}
...
handleToggleAbsent(checkbox){const row=checkbox.closest('tr');if(row){row.classList.toggle('is-absent',checkbox.checked);}}}
