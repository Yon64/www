...
_buildHTML(heat,config){const detailPortes=config.detailPortes||null;const zones=ConfigurationPenalites.genererSequenceZones(config.gateCount,config.rollZoneAfterGate,detailPortes);const zonesJugeesStr=heat.zonesJugees||'0'.repeat(zones.length);const isStartOk=heat.etat>=3;const activeSlots=heat.slots.filter(s=>s.participant&&s.participant.id);const hasActiveSlots=activeSlots.length>0;const allArrivalsEntered=hasActiveSlots&&activeSlots.every(s=>s.participant.ordre_arrivee||s.participant.specialStatus);const finishZoneIndex=zonesJugeesStr.length-1;const finishZoneJudged=finishZoneIndex>=0&&zonesJugeesStr[finishZoneIndex]!=='0';const isFinishOk=allArrivalsEntered||(heat.etat>=4);let stepsHtml='';if(!this.options.penaltyZonesOnly){stepsHtml+=this._createStepHtml('Dép.',isStartOk?'status-ok':'status-waiting','step-terminal');}
